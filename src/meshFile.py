...
class MeshFile:
    ...
    def processMesh(self, mesh: assimp_py.Mesh, scene: assimp_py.Scene) -> Mesh:
        ...
        for i in range(mesh.num_vertices):
            ...
            if (
                mesh.texcoords
                and mesh.texcoords[0]
                and mesh.tangents
                and mesh.bitangents
            ):
                vertices += (
                    list(mesh.texcoords[0][i][:2])
                    + list(mesh.tangents[i][:3])
                    + list(mesh.bitangents[i][:3])
                )
            else:
                vertices += [0] * (2 + 3 + 3)
        ...
        diffuseMaps = self.loadMaterialTextures(
            material, assimp_py.TextureType_DIFFUSE, "texture_diffuse"
        )
        textures += diffuseMaps
        specularMaps = self.loadMaterialTextures(
            material, assimp_py.TextureType_SPECULAR, "texture_specular"
        )
        textures += specularMaps
        normalMaps = self.loadMaterialTextures(
            material, assimp_py.TextureType_HEIGHT, "texture_normal"
        )
        textures += normalMaps
        heightMaps = self.loadMaterialTextures(
            material, assimp_py.TextureType_AMBIENT, "texture_height"
        )
        textures += heightMaps

        return Mesh(glm.array.from_numbers(glm.float32, *vertices), indices, textures)

    def loadMaterialTextures(self, mat: dict, type: int, typeName: str):

        textures = []
        for i in range(list(mat["TEXTURES"].keys()).count(type)):
            texStr = mat["TEXTURES"][type][i]
            if not list(
                filter(lambda texture: texture.path == texStr, self.textures_loaded)
            ):
...
def TextureFromFile(path: str, directory: str, gamma: bool = False) -> int:
    ...
    try:
        ...
        format = (
            GL_RED if nrComponents == 1 else GL_RGB if nrComponents == 3 else GL_RGBA
        )
