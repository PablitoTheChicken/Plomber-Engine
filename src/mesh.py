...
class Mesh:
    ...
    def __init__(
        self, vertices: glm.array, indices: List[int], textures: List[Texture]
    ):
    ...
    def setupMesh(self):
        ...
        glBufferData(
            GL_ARRAY_BUFFER, self.vertices.nbytes, self.vertices.ptr, GL_STATIC_DRAW
        )
        ...
        glVertexAttribPointer(
            1, 3, GL_FLOAT, GL_FALSE, Vertex.size, Vertex.NormalOffset
        )
        # vertex texture coords
        glEnableVertexAttribArray(2)
        glVertexAttribPointer(
            2, 2, GL_FLOAT, GL_FALSE, Vertex.size, Vertex.TexCoordsOffset
        )
        # vertex tangent
        glEnableVertexAttribArray(3)
        glVertexAttribPointer(
            3, 3, GL_FLOAT, GL_FALSE, Vertex.size, Vertex.TangentOffset
        )
        # vertex bitangent
        glEnableVertexAttribArray(4)
        glVertexAttribPointer(
            4, 3, GL_FLOAT, GL_FALSE, Vertex.size, Vertex.BitangentOffset
        )
